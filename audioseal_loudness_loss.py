...
if __name__ == "__main__":
    ...
    pertub_speaker_path = sys.argv[2]
    ...
    wav_tensor_initial = torch.from_numpy(wav).unsqueeze(0).unsqueeze(0).to(DEVICE)
    wav_tensor_initial.requires_grad = True
    
    # pertub_speaker
    print("Loading pertub speaker...")
    # ensure_default_models(Path(RTVC_DEFAULT_MODEL_PATH))
    # encoder.load_model(Path(RTVC_DEFAULT_MODEL_PATH + '/default/encoder.pt'))
    
    in_fpath = Path(pertub_speaker_path.replace("\"", "").replace("\'", ""))
    preprocessed_wav = encoder.preprocess_wav(in_fpath, SAMPLING_RATE)
    wav, _, mel_slices, RTVC_ENCODER_MODEL, _ = encoder.embed_utterance_preprocess(preprocessed_wav, using_partials=True)
    
    wav_tensor_pertub = torch.from_numpy(wav).unsqueeze(0).unsqueeze(0).to(DEVICE)
    wav_tensor_pertub.requires_grad = True
    
    # check shape of tensors
    print("Initial speaker tensor shape: ", wav_tensor_initial.shape)
    print("Pertub speaker tensor shape: ", wav_tensor_pertub.shape)
