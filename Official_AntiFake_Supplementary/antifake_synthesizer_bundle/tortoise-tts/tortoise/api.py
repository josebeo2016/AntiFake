    ...
    def get_conditioning_latents(self, voice_samples, return_mels=False):
        ...
        with torch.no_grad():
            ...
            for vs in voice_samples:
                auto_conds.append(format_conditioning(vs, device=self.device)) # torchvision mel spectrogram
            auto_conds = torch.stack(auto_conds, dim=1)
            self.autoregressive = self.autoregressive.to(self.device) 
            auto_latent = self.autoregressive.get_conditioning(auto_conds) # Transformer inside UnifiedVoice custom model to get conditioning
            ...
            for sample in voice_samples:
                ...
                cond_mel = wav_to_univnet_mel(sample.to(self.device), do_normalization=False, device=self.device) # tacotron mel spectrum encoder
                diffusion_conds.append(cond_mel)
            diffusion_conds = torch.stack(diffusion_conds, dim=1)

            self.diffusion = self.diffusion.to(self.device)
            diffusion_latent = self.diffusion.get_conditioning(diffusion_conds) # custom contextual embedder model
