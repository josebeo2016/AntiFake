...
if __name__ == "__main__":
    ...
    for target_speaker_path in target_speakers_selected:
        ...
        score_path = target_speaker_path.replace(".wav", ".npy")
        if os.path.exists(score_path):
            target_speaker_asv = np.load(score_path)
        else:
            target_speaker_wav, _ = librosa.load(target_speaker_path, sr=SAMPLING_RATE)
            target_speaker_asv = speaker_asv(target_speaker_wav)
            np.save(score_path, target_speaker_asv)
        
        asv_score_diff.append(torch.nn.functional.cosine_similarity(source_speaker_asv, target_speaker_asv))
